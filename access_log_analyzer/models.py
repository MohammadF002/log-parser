...
class AnalysisResult:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.error_requests <= self.valid_requests:
            raise ValueError("error requests must be within the valid request count")
        if not 0 <= self.unique_ip_count <= self.valid_requests:
            raise ValueError("unique IP count must be within the valid request count")

    @property
    def error_rate_percent(self) -> float:
        if self.valid_requests == 0:
            return 0.0
        return self.error_requests / self.valid_requests * 100
